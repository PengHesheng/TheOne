...
class NewFriendDao(config: DaoConfig, daoSession: DaoSession):
    ...
    object Properties {
        ...
        val Status = Property(5, Int::class.java, "status", false, "STATUS")
    ...
    override fun readKey(p0: Cursor?, p1: Int): Long? {
        return if (p0?.isNull(p1 + 0)!!) null else p0.getLong(p1 + 0)
    }
...
class DaoSession(db: SQLiteDatabase, type: IdentityScopeType,
    ...
    private val newFriendDaoConfig: DaoConfig = daoConfigMap[NewFriendDao::class.java]?.clone()!!
    private val newFriendDao: NewFriendDao
